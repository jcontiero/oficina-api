from src.shared.aplicacao.unit_of_work import UnitOfWork, transactional
import re
...
class CadastrarCliente:
    def __init__(self, repo: ClienteRepositorio, uow: UnitOfWork | None = None):
        self.repo = repo
...
class AtualizarVeiculo:
    def __init__(self, repo: VeiculoRepositorio, uow: UnitOfWork | None = None):
        self.repo = repo
...
class BuscarOrdemDeServico:
    ...
    def executar(self, id: UUID, cliente_id: UUID | None = None) -> OrdemDeServico:
...
class IniciarDiagnostico:
    def __init__(self, repo: OrdemDeServicoRepositorio, uow: UnitOfWork | None = None):
        self.repo = repo
    ...
    def executar(self, os_id: UUID) -> OrdemDeServico:
        ...
        if self.peca_repo:
            for item in os.itens_peca:
                peca = self.peca_repo.buscar_por_id_com_lock(item.peca_id)
                if peca:
...
class ListarOrdensDeServicoAtivas:
    ...
    def executar(self, cliente_id: UUID | None = None) -> list[OrdemDeServico]:
        from src.atendimento.dominio.value_objects import (
            StatusOSFase2,
            para_status_fase2,
        ...
        todas = self.repo.listar(cliente_id=cliente_id)
        ativas = [
            os
            for os in todas
