...
class Cliente:
    ...
    def inativar(self) -> None:
        self.status = StatusCliente.INATIVO
...
class Veiculo:
    ...
    def inativar(self) -> None:
        self.status = StatusVeiculo.INATIVO
...
class OrdemDeServico:
    ...
    historico: list[HistoricoOS] = field(default_factory=list)

    def __post_init__(self):
        if not self.historico and self.status == StatusOS.RECEBIDA:
            hist = HistoricoOS(
                id=uuid4(),
                os_id=self.id,
