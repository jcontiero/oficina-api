...
def upgrade():
    ...
    op.create_check_constraint(
        "ck_pecas_estoque", "pecas", "quantidade_disponivel >= 0"
    )
    op.create_check_constraint(
        "ck_item_servico_preco", "itens_servico", "preco_unitario >= 0"
    )
    op.create_check_constraint(
        "ck_item_peca_preco", "itens_peca", "preco_unitario >= 0"
    )
    ...
    # Indices
    op.create_index(
        "ix_historico_os_os_id_ocorrido", "historico_os", ["os_id", "ocorrido_em"]
    )
    ...
    op.create_index(
        "ix_os_status_criada_em", "ordens_de_servico", ["status", "criada_em"]
    )
