...
def upgrade():
    ...
    op.add_column('clientes', sa.Column('status', sa.String(length=20), server_default='ATIVO', nullable=False))
    op.add_column('veiculos', sa.Column('status', sa.String(length=20), server_default='ATIVO', nullable=False))
    ...
    op.create_table(
        ...
        sa.Column('status_anterior', sa.Enum('RECEBIDA', 'EM_DIAGNOSTICO', 'AGUARDANDO_ORCAMENTO', 'AGUARDANDO_APROVACAO', 'EM_EXECUCAO', 'SERVICOS_CONCLUIDOS', 'FINALIZADA', 'ENTREGUE', 'CANCELADA', name='status_os'), nullable=True),
        sa.Column('status_novo', sa.Enum('RECEBIDA', 'EM_DIAGNOSTICO', 'AGUARDANDO_ORCAMENTO', 'AGUARDANDO_APROVACAO', 'EM_EXECUCAO', 'SERVICOS_CONCLUIDOS', 'FINALIZADA', 'ENTREGUE', 'CANCELADA', name='status_os'), nullable=False),
        ...
        sa.ForeignKeyConstraint(['os_id'], ['ordens_de_servico.id'], ),
